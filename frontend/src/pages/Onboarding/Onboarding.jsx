...
const Onboarding = () => {
  ...
  const toggleCategory = (cat) => {
    ...
    handlePersonalChange('pwdCrossCategory', cat === 'PWD (Cross-category applicant)');
  };
  ...
  const handleContinue = async () => {
    ...
    if (activeStep === 4) {
      try {
        const pendingRaw = sessionStorage.getItem('signup_pending_credentials');
        const pendingCreds = pendingRaw ? JSON.parse(pendingRaw) : {};

        const payload = {
          name: studentProfile.fullName || personal.fullName,
          fullName: studentProfile.fullName || personal.fullName,
          email: studentProfile.email || personal.email,
          category: studentProfile.category || personal.category,
          pwdCrossCategory: studentProfile.pwdCrossCategory || personal.pwdCrossCategory,
          phone: studentProfile.phone || personal.phone,
          domicile: studentProfile.domicile || personal.domicile,
          exam: studentProfile.academic?.exam || academic.exam,
          examScore: studentProfile.academic?.examScore || academic.examScore,
          careerOption: studentProfile.academic?.careerOption || academic.careerOption,
          preferredBranch: studentProfile.academic?.preferredBranch || academic.preferredBranch,
          preferredLocation: studentProfile.preferences?.preferredLocation || preferences.preferredLocation,
          budgetRange: studentProfile.preferences?.budgetRange || preferences.budgetRange,
          collegeType: studentProfile.preferences?.collegeType || preferences.collegeType,
          hostelRequired: studentProfile.preferences?.hostelRequired ?? preferences.hostelRequired,
        };

        if (pendingCreds?.password) {
          payload.password = pendingCreds.password;
          sessionStorage.setItem('signup_complete_payload', JSON.stringify(payload));
          sessionStorage.setItem('auth_pending_user', JSON.stringify({
            name: payload.name,
            email: payload.email,
            phone: payload.phone,
          }));
          sessionStorage.setItem('auth_pending_phone', payload.phone);
        ...
        const detail = error?.response?.data?.detail || error?.response?.data?.message || error?.message || 'Failed to save profile. Please try again.';
        toast.error(detail);
      }
    }
  };
  ...
  return (
    <div className="onboarding-wrapper">
      ...
      <section className="form-section">
        ...
        <div className="bottom-action" style={{ display: 'flex', gap: '12px', justifyContent: 'center' }}>
          {activeStep > 1 && (
            <button
              type="button"
